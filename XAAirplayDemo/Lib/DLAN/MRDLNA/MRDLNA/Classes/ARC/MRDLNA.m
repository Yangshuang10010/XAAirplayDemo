...
-(void)initCLUPnPRendererAndDlnaPlay
{
    ...
    [self.render setAVTransportURL:self.playUrl startPositon:self.startPosition];
}
...
{
    if (![self trimEmpty:speed]) {
        [self.render playWithSpeed:speed];
    }
}
...
- (void)playTheURL:(NSString *)url startPositon:(CGFloat)relTime
{
    self.playUrl = url;
    if (self.render) {
        [self.render setAVTransportURL:url startPositon:relTime];
    }else {
        [self startDLNAAfterStop];
    }
}
...
- (void)upnpSetAVTransportURIResponse{
    [self.render play];
    if (self.delegate && [self.delegate respondsToSelector:@selector(upnpSetAVTransportURIResponse)]) {
...
//        [self.render play];
//    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(upnpGetTransportInfoResponse:)]) {
...
#pragma mark GetPositionInfoResponse
