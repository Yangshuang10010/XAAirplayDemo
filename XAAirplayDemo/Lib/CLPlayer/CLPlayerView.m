...
- (void)horizontalMoved:(CGFloat)value {
    ...
    //滑杆进度
    _sliderValue        = dragedSeconds / totalMovieDuration;
    //设置滑杆
    self.maskView.slider.value = _sliderValue;
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(cl_progressValueDidChanged:)]) {
        [_delegate cl_progressValueDidChanged:totalMovieDuration*self.maskView.slider.value];
    }
}
...
-(void)cl_progressSliderValueChanged:(CLSlider *)slider{
    ...
    _sliderValue        = dragedSeconds / total;
    //转换成CMTime才能给player来控制播放进度
    CMTime dragedCMTime     = CMTimeMake(dragedSeconds, 1);
    [_player seekToTime:dragedCMTime toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
    NSInteger proMin                    = (NSInteger)CMTimeGetSeconds(dragedCMTime) / 60;//当前秒
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(cl_progressValueDidChanged:)]) {
           [_delegate cl_progressValueDidChanged:dragedSeconds];
       }
}
...
- (void)timeStack{
    if (_playerItem.duration.timescale != 0){
        ...
        _sliderValue = self.maskView.slider.value;
        //判断是否真正的在播放
