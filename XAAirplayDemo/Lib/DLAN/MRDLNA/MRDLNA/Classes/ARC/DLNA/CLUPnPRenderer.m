...
- (void)setAVTransportURL:(NSString *)urlStr startPositon:(CGFloat)relTime{
    CLUPnPAction *action = [[CLUPnPAction alloc] initWithAction:@"SetAVTransportURI"];
    [action setArgumentValue:@"0" forName:@"InstanceID"];
    [action setArgumentValue:urlStr forName:@"CurrentURI"];
    [action setArgumentValue:VideoDIDL forName:@"CurrentURIMetaData"];
    [action setArgumentValue:[NSString stringWithDurationTime:relTime] forName:@"CurrentRelTime"];
    NSLog(@"CurrentRelTime%@", [NSString stringWithDurationTime:relTime]);
    [self postRequestWith:action];
}
...
- (void)play{
    [self playWithSpeed:@"1"];
}

-(void)playWithSpeed:(NSString *)speed
{
    //speed无效
    CLUPnPAction *action = [[CLUPnPAction alloc] initWithAction:@"Play"];
    [action setArgumentValue:@"0" forName:@"InstanceID"];
    [action setArgumentValue:speed forName:@"Speed"];
    [self postRequestWith:action];
}
